...
export const objectIdSchema = () => stringSchema().refine((id) => mongoose.Types.ObjectId.isValid(id), 'Invalid ObjectId format');
...
export const enumSchema = (list) => z.enum(list);
export const phoneSchema = () => z
    .string()
    .trim()
    .regex(/^\+?[1-9]\d{7,14}$/, 'Enter a valid phone number (e.g. +2348012345678)');
export const accountNumberSchema = () => stringSchema()
    .trim()
...
export const createFileSchema = ({ maxSizeMB }) => {
    if (isServer) {
        return z.object({
            fieldname: z.string(),
            ...
            size: z.number().max(maxSizeMB * 1024 * 1024, `File must be under ${maxSizeMB}MB`),
...
export const numberSchema = (error) => z.preprocess((value) => {
