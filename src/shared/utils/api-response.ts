...
export class ApiResponse {
  ...
  static paginated<T>(
    res: Response, data: T[], meta: PaginationMeta, message = 'Success'
  ) {
    const body: ApiResponseBody<T[]> = { success: true, message, data, meta };
    return res.status(StatusCodes.OK).json(body);
  }

  static error(res: Response, message: string, statusCode = StatusCodes.BAD_REQUEST, errors?: string[] | { field: string; message: string }[]) {
